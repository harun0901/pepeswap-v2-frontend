...
const minimumPepeToRegister = new BigNumber(REGISTER_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)
...
const UserName: React.FC = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumPepeRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumPepeRequired = useHasPepeBalance(minimumPepeToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumPepeRequired={minimumPepeRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumPepeRequired && (
        <Text color="failure" mt="16px">
          {t('A minimum of %num% PEPE is required', { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
