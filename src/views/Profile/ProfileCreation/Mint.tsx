...
import { usePepe, useBunnyFactory } from 'hooks/useContract'
import { Nft } from 'config/constants/types'
import useHasPepeBalance from 'hooks/useHasPepeBalance'
...
const minimumPepeBalanceToMint = new BigNumber(MINT_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)

const Mint: React.FC = () => {
  const [variationId, setVariationId] = useState<Nft['variationId']>(null)
  const { actions, minimumPepeRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const pepeContract = usePepe()
  const bunnyFactoryContract = useBunnyFactory()
  const { t } = useTranslation()
  const hasMinimumPepeRequired = useHasPepeBalance(minimumPepeBalanceToMint)
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        // TODO: Move this to a helper, this check will be probably be used many times
        try {
          const response = await pepeContract.methods.allowance(account, bunnyFactoryContract.options.address).call()
          const currentAllowance = new BigNumber(response)
          return currentAllowance.gte(minimumPepeRequired)
        } catch (error) {
          return false
        }
      },
      onApprove: () => {
        return pepeContract.methods
          ...
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumPepeRequired}
              >
                <Text bold>{nft.name}</Text>
              </SelectionCard>
            )
          })}
          {!hasMinimumPepeRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% PEPE is required', { num: MINT_COST })}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={variationId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumPepeRequired}
