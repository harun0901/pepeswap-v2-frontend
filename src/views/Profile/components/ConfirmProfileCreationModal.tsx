...
import { usePepe, useProfile } from 'hooks/useContract'
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        try {
          const response = await pepeContract.methods.allowance(account, profileContract.options.address).call()
      ...
      onApprove: () => {
        return pepeContract.methods.approve(profileContract.options.address, allowance.toJSON()).send({ from: account })
      },
      onConfirm: () => {
  ...
  return (
    <Modal title={t('Complete Profile')} onDismiss={onDismiss}>
      ...
      <Flex justifyContent="space-between" mb="16px">
        <Text>{t('Cost')}</Text>
        <Text>{t('%num% PEPE', { num: REGISTER_COST })}</Text>
      </Flex>
