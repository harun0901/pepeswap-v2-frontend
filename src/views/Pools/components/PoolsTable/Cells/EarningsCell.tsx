...
const EarningsCell: React.FC<EarningsCellProps> = ({ pool, account, userDataLoaded }) => {
  ...
  // These will be reassigned later if its Auto PEPEPEPEPEPE vault
  ...
  const labelText = isAutoVault ? t('Recent PEPE profit') : t('%asset% Earned', { asset: earningToken.symbol })
  earningTokenBalance = isAutoVault ? autoPepeToDisplay : earningTokenBalance
  hasEarnings = isAutoVault ? hasAutoEarnings : hasEarnings
  earningTokenDollarBalance = isAutoVault ? autoUsdToDisplay : earningTokenDollarBalance

  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      <Balance fontSize="16px" value={autoPepeToDisplay} decimals={3} bold unit=" PEPE" />
  ...
  const [onPresentCollect] = useModal(
    <CollectModal
      ...
      sousId={sousId}
      isBnbPool={isBnbPool}
      isCompoundPool={isManualPepePool}
    />,
  )
