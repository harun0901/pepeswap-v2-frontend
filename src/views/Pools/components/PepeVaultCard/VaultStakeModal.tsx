...
import { fetchPepeVaultUserData } from 'state/pools'
import { Pool } from 'state/types'
import { getAddress } from 'utils/addressHelpers'
import { convertPepeToShares } from '../../helpers'
...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({ pool, stakingMax, isRemovingStake = false, onDismiss }) => {
  ...
  const cakeVaultContract = usePepeVaultContract()
  const {
  ...
  const usdValueStaked =
    pepePriceBusd.gt(0) && stakeAmount ? formatNumber(new BigNumber(stakeAmount).times(pepePriceBusd).toNumber()) : ''
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    setPendingTx(true)
    const shareStakeToWithdraw = convertPepeToShares(convertedStakeAmount, pricePerFullShare)
    // trigger withdrawAll function if the withdrawal will leave 0.000001 PEPE or less
  ...
  return (
    ...
    >
      ...
      <BalanceInput
        ...
        currencyValue={pepePriceBusd.gt(0) && `~${usdValueStaked || 0} USD`}
