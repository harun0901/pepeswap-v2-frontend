...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  const pepeCosts = (amount: string): number => {
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        max={fullBalance}
        symbol="TICKET"
        availableSymbol="PEPE"
      />
      <div>
        <Tips>{t(`1 Ticket = ${LOTTERY_TICKET_PRICE} PEPE`, { num: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t(
            'Ticket purchases are final. Your PEPE cannot be returned to you after buying tickets.',
          )}
        </Announce>
        <Final>{t(`You will spend: ${pepeCosts(val)} PEPE`)}</Final>
      </div>
      <ModalActions>
