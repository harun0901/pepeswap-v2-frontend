...
const PrizeGrid: React.FC<PrizeGridProps> = ({
  ...
  const pepeBusdPrice = usePricePepeBusd()

  const getPepeBusdValue = (amount: number) => {
    return new BigNumber(amount).multipliedBy(pepeBusdPrice).toNumber()
  }
    ...
    <Grid pastDraw={pastDraw}>
      ...
      <GridItem>
        <RightAlignedHeading size="md">
          ...
          {!pastDraw && !pepeBusdPrice.eq(0) && <CardBusdValue value={getPepeBusdValue(fourMatchesAmount)} />}
        </RightAlignedHeading>
      </GridItem>
      {/* 3 matches row */}
      <GridItem>
      ...
      <GridItem>
        <RightAlignedText>
          ...
          {!pastDraw && !pepeBusdPrice.eq(0) && <CardBusdValue value={getPepeBusdValue(threeMatchesAmount)} />}
        </RightAlignedText>
      </GridItem>
      {/* 2 matches row */}
      <GridItem>
      ...
      <GridItem>
        <RightAlignedText>
          ...
          {!pastDraw && !pepeBusdPrice.eq(0) && <CardBusdValue value={getPepeBusdValue(twoMatchesAmount)} />}
        </RightAlignedText>
      </GridItem>
      {/* Burn row */}
