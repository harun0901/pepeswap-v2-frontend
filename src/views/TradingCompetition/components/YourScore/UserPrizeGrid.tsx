...
import { useCompetitionPepeRewards, getRewardGroupAchievements } from '../../helpers'
...
const UserPrizeGrid: React.FC<{ userTradingInformation?: UserTradingInformationProps }> = ({
  ...
  const { userRewardGroup, userPepeRewards, userPointReward, canClaimNFT } = userTradingInformation
  const { pepeReward, dollarValueOfCakeReward } = useCompetitionPepeRewards(userPepeRewards)
        ...
        <tr>
          <BoldTd>
            <Flex flexDirection="column">
              <Text bold>{pepeReward.toFixed(2)}</Text>
