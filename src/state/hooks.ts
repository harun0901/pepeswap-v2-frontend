...
export const usePollCoreFarmData = () => {
  ...
  useEffect(() => {
    dispatch(fetchFarmsPublicDataAsync([3, 5]))
...
export const useFetchPepeVault = () => {
  const { account } = useWeb3React()
  const { fastRefresh } = useRefresh()
  const dispatch = useAppDispatch()

  useEffect(() => {
    dispatch(fetchPepeVaultPublicData())
  }, [dispatch, fastRefresh])

  useEffect(() => {
    dispatch(fetchPepeVaultUserData({ account }))
  }, [dispatch, fastRefresh, account])

  useEffect(() => {
    dispatch(fetchPepeVaultFees())
  }, [dispatch])
}

export const usePepeVault = () => {
  const {
    totalShares: totalSharesAsString,
    pricePerFullShare: pricePerFullShareAsString,
    totalPepeInVault: totalPepeInVaultAsString,
    estimatedPepeBountyReward: estimatedPepeBountyRewardAsString,
    totalPendingPepeHarvest: totalPendingPepeHarvestAsString,
    fees: { performanceFee, callFee, withdrawalFee, withdrawalFeePeriod },
    userData: {
      isLoading,
      userShares: userSharesAsString,
      pepeAtLastUserAction: pepeAtLastUserActionAsString,
      lastDepositedTime,
      lastUserActionTime,
    },
  } = useSelector((state: State) => state.pools.pepeVault)

  const estimatedPepeBountyReward = useMemo(() => {
    return new BigNumber(estimatedPepeBountyRewardAsString)
  }, [estimatedPepeBountyRewardAsString])

  const totalPendingPepeHarvest = useMemo(() => {
    return new BigNumber(totalPendingPepeHarvestAsString)
  }, [totalPendingPepeHarvestAsString])
  ...
  const totalPepeInVault = useMemo(() => {
    return new BigNumber(totalPepeInVaultAsString)
  }, [totalPepeInVaultAsString])

  const userShares = useMemo(() => {
    return new BigNumber(userSharesAsString)
  }, [userSharesAsString])

  const pepeAtLastUserAction = useMemo(() => {
    return new BigNumber(pepeAtLastUserActionAsString)
  }, [pepeAtLastUserActionAsString])
...
export const usePricePepeBusd = (): BigNumber => {
  const pepeBnbFarm = useFarmFromPid(3)
  return new BigNumber(pepeBnbFarm.token.busdPrice)
}
